...
export default class Message extends React.Component {
  ...
  sendMessage = async message => {
    ...
    newReference.set(params).then(() => this.onSendApi())
  }
  onSendApi = async () => {
    const config = {
    ...
    database()
      ...
      .on('value', snapshot => {
        ...
        if (typeof snapshot.val()[this.state.user2_id] !== 'undefined') {
          if (
            typeof snapshot.val()[this.state.user2_id].typing !== 'undefined'
          ) {
            this.setState({
        ...
        <View
                ...
                return (
                  <View>
                    {item.sender_id == this.state.user1_id
                      ? this.senderMessage(item, index)
                      : this.receverMessage(item, index)}
                  </View>
                )
              }}
            />
          ...
          <View
            style={{
              ...
              flexDirection: 'row',
              marginBottom:
                Platform.OS == 'ios' ? this.state.keyboardOffset : 0,
            }}>
            <TextInput
              style={{
                height: 40,
